...
AardvarkI2cFlags AardvarkI2C::toAardvarkFlags(I2cFlags flag)
{
	switch (flag)
	{
	case I2cFlags::None:   return AA_I2C_NO_FLAGS;
	case I2cFlags::NoStop: return AA_I2C_NO_STOP;
	default:               return AA_I2C_NO_FLAGS;
	}
}
...
std::unique_ptr<AardvarkI2C> AardvarkI2C::open(const AaPortInfo& info, bool printDetails)
{
	...
	{
	case AaConfig::ID_PFWD:	   tempHandle.name = "pFWD"; break;
	case AaConfig::ID_P45:	   tempHandle.name = "p45 "; break;
	case AaConfig::ID_PST_IMU: tempHandle.name = "pStat"; break;
	default:				   tempHandle.name = "Unknown AA";
	}
	...
	{
		return errorHandler("Error during configuaration.", tempHandle.config);
	}
